...
parser.add_argument('--dataset', default='Tree_cycle')
...
if 'cycle' in args.dataset.lower():
    nclass = 2
...
if args.mode == 'train':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        loss = criterion(logits[train_mask], data.y[train_mask].long())# 손실함수 계산
...
for size in sizes:
    for version in versions:
        with open(f'../dataset/Tree/{args.dataset}_{size}_{version}.pkl', 'rb') as fin:
            data = pickle.load(fin)
            data.to(device)
        model.eval()
