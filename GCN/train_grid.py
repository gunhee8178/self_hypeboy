...
if args.mode == 'train':
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        loss = criterion(logits, data.y.long())# 손실함수 계산
...
for size in sizes:
    for version in versions:
        ...
        # acc = int(data == pred).sum() ) / len(test_indices)
        acc = (data.y[:511] == pred[:511]).sum() / len(data.y[:511])
        a = 0
        b = 0
        c = 0
        print("{} acc: {} {} {}".format('final', acc,  (data.y[:511] == pred[:511]).sum(), len(data.y[:511])))
        for i in range(4):
            a += (pred[data.node_mask]==i).sum()
            b += len(torch.where((pred[data.node_mask]==i) & (data.y[data.node_mask]==i))[0])
            print(f'c: {(pred[511:]==i).sum()}')
